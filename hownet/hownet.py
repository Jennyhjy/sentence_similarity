...
class Atom(object):
    ...
    def __init__(self, part=None):
        self.logic = None # ~^ 在 义原 或者 符号之前
        self.rel_primitive = None # 关系义原英文部分
        self.rel_symbol = None # #%$*+&@?! 关系符号
        self.eng = None # 义原英文部分
        self.chi = None # 义原中文部分
        self.is_concrete = False # 具体词
...
def similar_sentence(sent1, sent2):
    similar = 0.0
    for w1 in sent1:
        for w2 in sent2:
            score = similar_word(w1, w2)
            # print('{}, {}, {}'.format(w1, w2, score))
            similar += score
    return similar/(len(sent1)*len(sent2))
...
if __name__ == '__main__':
   ...
   #         print(t)
   print(similar_word('水稻','小麦'))#0.861
...
#   print('=====================')
   print(similar_sentence('科学家 水稻 教师'.split(), '小麦 运动员 教师'.split()))  # 0.6
